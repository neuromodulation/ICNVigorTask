...
for i in range(n_dataset):
    percentile_stim[i] = np.nanmedian([percentileofscore(feature_matrix_nn_start[i, 1, :], x) for x in feature_matrix_nn_start[i, 1, :][stim_start[i, 1, :] == 1]])
...
plt.subplot(1, 2, 2)
x = np.nanmedian(feature_matrix[:, 1, :n_moves], axis=1)
y = percentile_stim
corr, p = spearmanr(x, y)
sb.regplot(x=x, y=y)
plt.title(f"corr = {np.round(corr, 2)}, p = {np.round(p, 3)}", fontweight='bold')
feature_name_space = feature_name.replace("_", " ")
plt.xlabel(f"Median \n in {feature_name_space} in first {n_moves} moves [%]", fontsize=12)
plt.ylabel(f"Percentile of {feature_name_space} \n fast stim in first {n_moves} moves", fontsize=12)
