# Script for orderition order analysis
...
order_names = ["Slow-Fast", "Fast-Slow"]
for order in range(2):
    ...
    median_peak_speed = np.median(peak_speed[idx_order,:,:,:], axis=0)

    # Compute standard deviation over all datasets
    std_peak_speed = np.std(peak_speed[idx_order,:,:,:], axis=0)
