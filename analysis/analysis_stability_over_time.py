# Script for change in stability/correlation over time
...
from ICNVigorTask.utils.utils import reshape_data_trials, norm_speed, smooth_moving_average, plot_speed, \
    fill_outliers, norm_perf_speed, norm_0_1, get_bids_filepath, moving_variance, get_peak_acc, get_onset_idx,\
    compute_difference_over_time
...
with alive_bar(len(subject_list), force_tty=True, bar='smooth') as bar:
    for subject in subject_list:
        ...
        data = data[:, :, :, mean_speed_idx, :]
        def align_onset(array):
            #start_idx = np.argmax(array)
            start_idx = np.where(array > 500)[0][0]
            array_tmp = array[start_idx:]
            new_array = np.zeros(array.shape)
            new_array[:len(array_tmp)] = array_tmp
            return new_array

        np.apply_along_axis(lambda m: align_onset(m), axis=3, arr=data)

        # Compute similarity if subsequent speed curves
        diff = compute_difference_over_time(data)

        # Detect and fill outliers (e.g. when subject did not touch the screen)
        np.apply_along_axis(lambda m: fill_outliers(m), axis=2, arr=diff)
        np.apply_along_axis(lambda m: fill_outliers(m), axis=2, arr=diff)
