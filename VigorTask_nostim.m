...
options.filename = '491LR57_plasticity';
options.ntrials = 15;
options.nblocks = 9;
options.target_size_x = 200;
options.target_size_y = 200;
...
elseif strcmp(options.system,'AO')
    ...
    AO_SetSaveFileName(options.filename);AO_StartSave();
...
%             cgpencol(1,1,1)
            cgellipse(x,y,15,15,'f')
            cgflip(0,0,0);
            if km(1)
                pause(.1);km=cgkeymap;
                save(options.filename,'-v7.3')
