...
if use_tmsi 
    ...
    device =  library.getFirstAvailableDevice('usb');
    device.connect();
    device.start();
...
for i_block=1:n_blocks
    ...
    for i_trial=1:n_trials_block
        ...
        while ~trial_completed
            ...
            if use_tmsi 
                [samples_tmsi, num_sets, type] = device.sample();
            end
            ...
            if ~(ismember(i_block, break_blocks) && i_trial == 1)
                ...
                if use_tmsi
                    ...
                    sample_length_tmsi = cat(1, sample_length_tmsi, size(samples_tmsi, 2));
                end 
            ...
            % the mean velocity decreased during the last 3 samples --> the
            % peak is passed
...
if ~test
    ...
    if use_tmsi
        ...
        data_tmsi = cat(2,data_tmsi.',data_upsample);
        file_name = strcat("sub-",string(n_par),"-",med,"-task-VigorStim-",hand,...
                            "-", conditions(cond+1),"-StimOn","-run-0",string(run),...
                            "-neuro.mat");
        save(strcat(pwd,'\Data\',file_name), "data_tmsi");
    end
end