...
% TODO: Use first stim onset to generate an array onatining the events

clear all;
%% Synchronize TMSi data
% Load the behavioural data
[filename_behav,path_behav] = uigetfile(strcat(pwd,'/Data/Parkinson/'));
load(strcat(path_behav,filename_behav));
behav_data = struct.data;
% Load the TMSi data
[filename,path] = uigetfile(fullfile(strcat(pwd,'/Data/Parkinson/'),'*.Poly5'));
