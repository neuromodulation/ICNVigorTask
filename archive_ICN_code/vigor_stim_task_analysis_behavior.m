    ...
    for i_block=1:n_blocks
        ...
        for i_trial=1:n_trials
            ...
            if i_block == 1
                ...
                if stim == 0 & true_stim == 1
                    subplot(3,1,1);
                    plot(data_trial(:,4)); hold on; 
                    plot(data_trial(:,5)); 
                    mask = data(:,8) == stim_blocks(i_cond,i_block) & data(:,9) == i_trial-1;
                    data_trial_pr = data(mask,:); 
                    subplot(3,1,2);
                    plot(data_trial_pr(:,5)); hold on;
                    xline(data_trial(ind_stim,3));
                    %xline(data_trial(ind_peak,3));
                    x = 3
                end
            end
        end
...
subplot(3,1,2);
diffs_stim_peak(diffs_stim_peak < -0.1) = -0.1;
histogram(diffs_stim_peak,30);
